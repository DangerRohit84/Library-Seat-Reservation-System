...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({ user, onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-dark-950 pb-20 relative text-slate-200">
      
      {/* Toast */}
      {toast && (
        <div className={`fixed top-4 right-4 z-50 px-6 py-3 rounded-xl shadow-lg border flex items-center gap-3 animate-in fade-in slide-in-from-top-5 ${toast.type === 'success' ? 'bg-dark-900 border-green-900 text-green-400' : 'bg-dark-900 border-red-900 text-red-400'}`}>
      ...
      {isLoading && (
         <div className="fixed inset-0 bg-dark-950/70 backdrop-blur-sm z-40 flex items-center justify-center">
            <Loader2 className="w-8 h-8 text-primary animate-spin" />
         </div>
      )}

      {/* Header */}
      <header className="bg-dark-900 border-b border-slate-800 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center text-white font-bold shadow-lg shadow-primary/20">L</div>
            <span className="font-bold text-white hidden sm:block">LibBook Student</span>
          </div>
          <div className="flex items-center gap-4">
            <span className="text-sm text-slate-400 hidden sm:block">Welcome, {user.name}</span>
            <button onClick={onLogout} className="p-2 text-slate-400 hover:text-red-400 hover:bg-red-900/20 rounded-full transition-colors">
              <LogOut size={20} />
            </button>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Tabs */}
        <div className="flex space-x-1 bg-dark-900 p-1 rounded-xl shadow-sm max-w-md mx-auto mb-8 border border-slate-800">
          <button
            onClick={() => setActiveTab('book')}
            className={`flex-1 flex items-center justify-center py-2.5 text-sm font-medium rounded-lg transition-all ${
              activeTab === 'book' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center py-2.5 text-sm font-medium rounded-lg transition-all ${
              activeTab === 'history' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-400 hover:text-white'
            }`}
          >
        ...
        {activeTab === 'book' && (
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {filteredSlots.length > 0 ? (
                      ...
                      <div className="p-4 bg-dark-950 rounded-lg text-center text-xs text-slate-500 border border-slate-800">
              ...
              {selectedSeat && (
                <div className="bg-dark-900 p-6 rounded-2xl shadow-lg border border-primary/40 animate-in fade-in slide-in-from-bottom-4 relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-20 h-20 bg-primary/10 rounded-full blur-xl -translate-y-1/2 translate-x-1/2"></div>
                  
                  <h3 className="font-bold text-white mb-1">Confirm Booking</h3>
                  <p className="text-sm text-slate-400 mb-4">Please review your selection</p>
                  
                  <div className="space-y-3 mb-6 bg-dark-950 p-4 rounded-xl border border-slate-800">
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-500">Seat</span>
                      <span className="font-semibold text-white">{selectedSeat.label} <span className="text-xs font-normal text-slate-500">({selectedSeat.type})</span></span>
                    </div>
                    <div className="flex justify-between text-sm">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-white">{ALL_TIME_SLOTS.find(t => t.value === selectedTime)?.label || 'N/A'}</span>
                    </div>
                  </div>

                  <button 
                    onClick={handleBook}
                    className="w-full py-3 bg-primary hover:bg-violet-600 text-white font-semibold rounded-xl shadow-lg shadow-primary/20 transition-all active:scale-[0.98]"
                  >
                    Confirm Booking
                  </button>
                </div>
            ...
            <div className="lg:col-span-2">
               <div className="bg-dark-900 p-1 rounded-2xl shadow-lg border border-slate-800 overflow-hidden">
                 <div className="p-4 border-b border-slate-800 flex justify-between items-center">
                    <h2 className="font-semibold text-slate-300">Seat Map</h2>
                    <span className="text-xs bg-dark-950 text-slate-400 px-3 py-1 rounded-full border border-slate-800">
                  ...
                  <tbody className="divide-y divide-slate-800 text-slate-300">
                    {myBookings.map((booking) => (
                      <tr key={booking.id} className="hover:bg-slate-800/30">
                        ...
                        <td className="px-6 py-4 font-semibold text-white">{seats.find(s => s.id === booking.seatId)?.label || 'Unknown'}</td>
                        ...
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border
                            ${booking.status === 'ACTIVE' ? 'bg-green-900/30 text-green-400 border-green-900' : 
                              booking.status === 'CANCELLED' ? 'bg-red-900/30 text-red-400 border-red-900' : 
                        ...
                        <td className="px-6 py-4">
                          {booking.status === 'ACTIVE' && (
                            <button 
                              ...
                              className="text-red-400 hover:text-red-300 hover:bg-red-900/30 p-1.5 rounded-md transition-colors"
